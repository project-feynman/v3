<template>
  <div style="display: flex; align-items: center;">
    <BaseButton color="grey darken-2" :filled="isColorActive(color)" @click="selectColor(color)">
    ...
    <div>
      ...
      <template v-for="(color, i) in colors">
        <span @click="changePenColor(color, i)"
          class="dot" 
          :style="`background-color: ${color}; margin-left: 2px; margin-right: 2px;`"
          :key="i"
        />
      </template>
    </div>
...
export default {
  ...
  computed: {
    ...
    color () {
      return this.user.currentPenColor;
    }
  },
  methods: {
    isColorActive (color) {
